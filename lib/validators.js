 ...
 * @param {array} validAssets - Array of valid asset symbols (optional)
 * @returns {string} Validated asset (uppercase)
 */
function validateAsset(asset, validAssets = ['USDC', 'USDT', 'cbBTC', 'WBTC', 'tBTC', 'ETH', 'WETH', 'DAI']) {
  if (!asset || asset === '') {
 ...
 */
function validateVaultName(vaultName, validVaults = ['jusdi', 'jbtci', 'jsoli', 'jeths']) {
...
/**
 * Validate a BOLT11 Lightning invoice
 * @param {string} invoice - Lightning payment request
 * @param {string} context - Context for error message
 * @returns {string} Validated invoice
 */
function validateLightningInvoice(invoice, context = 'Lightning invoice') {
  if (!invoice || invoice === '') {
    throw new ValidationError(`${context} is required`);
  }
  ...
  if (!lower.startsWith('lnbc') && !lower.startsWith('lntb') && !lower.startsWith('lnbcrt')) {
    throw new ValidationError(
      `Invalid ${context.toLowerCase()}. Must start with "lnbc" (mainnet), "lntb" (testnet), or "lnbcrt" (regtest).`
    );
  }
 ...
 * @param {string} mint - Mint address to validate
 * @param {string} context - Context for error message
 * @returns {string} Validated mint
 */
function validateSolanaMint(mint, context = 'Solana mint') {
  if (!mint || mint === '') {
    throw new ValidationError(`${context} is required`);
  }
  ...
  if (!base58Regex.test(mint)) {
    throw new ValidationError(
      `Invalid ${context.toLowerCase()}: "${mint}". Must be a valid base58-encoded Solana address.`
    );
  }
...
function displayValidationError(error, command) {
  ...
  const usageExamples = {
    ...
    swap: 'npm run swap <amount> <fromToken> <toToken> [chain]\n   Example: npm run swap 0.01 ETH USDC base',
    'uniswap-swap': 'npm run uniswap-swap <amount> <fromToken> <toToken> [chain]\n   Example: npm run uniswap-swap 0.01 ETH USDC base\n   Chains: ethereum, base',
    'jupiter-swap': 'npm run jupiter-swap <amount> <fromToken> <toToken>\n   Example: npm run jupiter-swap 1 SOL USDC\n   Tokens: SOL, USDC, USDT, JUP, BONK, JITOSOL, etc.',
    'lightning-pay': 'npm run lightning-pay pay <bolt11_invoice> [max_sats]\n   Example: npm run lightning-pay pay lnbc10u1p... 50000',
    'lightning-lnget': 'npm run lightning-pay lnget <url> [max_sats]\n   Example: npm run lightning-pay lnget https://api.example.com 500'
  };
