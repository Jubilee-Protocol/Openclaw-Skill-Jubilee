...
function getProvider(chain, config) {
  ...
  // config networks define `rpcUrl`; previously this read `network.rpc` and
  // passed `undefined` to JsonRpcProvider, which made every RPC call fail with
  // "failed to detect network". Accept both keys for backwards compatibility.
  const url = network.rpcUrl || network.rpc;
  if (!url) {
