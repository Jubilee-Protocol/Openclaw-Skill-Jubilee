...
async function generateWarRoom(chain = 'base') {
  try {
    ...
    // Section 2: Git Activity (optional, only if available)
    ...
    console.log(chalk.gray('Note: APY estimates are conservative averages, not real-time data'));
...
async function analyzeTreasury(wallet, provider, chain) {
  ...
  if (network.contracts.jUSDi) {
    ...
    // Sustainability check (using conservative estimate)
    const targetAPY = 0.05; // 5% conservative (estimated, not real-time)
    const monthlyYield = (value * targetAPY) / 12;
    const isSustainable = monthlyYield >= monthlyBurn;
    console.log(isSustainable 
      ? chalk.green('✓ IMMORTAL: Estimated yield covers burn rate') 
      : chalk.yellow('⚠ WARNING: Burn may exceed estimated yield')
    );
  }
...
function analyzeGitActivity() {
  try {
    ...
    // Get last 5 commits
    const commits = execSync('git log -5 --pretty=format:"%h - %s (%cr)"', { 
      encoding: 'utf-8',
    ...
    console.log(chalk.gray('Git not available or not in a repository'));
    console.log(chalk.gray('Skipping development activity analysis'));
  }
}
